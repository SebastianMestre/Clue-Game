...
void ui_init(
  ...
  *map = map_new(map_size, pasadizos);
  ...
  for(int i = 1; i < deck_suspects()->size; i++){
    player_pushCard(*player_arr + (player_idx%temp), deck_suspects()->data[randlist[i]]);
  ...
  for(int i = 1; i < deck_weapons()->size; i++){
    player_pushCard(*player_arr + (player_idx%temp), deck_weapons()->data[randlist[i]]);
    player_idx = player_idx + 1 % player_arr_size;
  }
  free(randlist);
  ...
  for(int i = 1; i < deck_scenes()->size; i++){
    player_pushCard(*player_arr + (player_idx%temp), deck_scenes()->data[randlist[i]]);
    player_idx = player_idx + 1 % player_arr_size;
  }
  free(randlist);
}
...
bool ui_manager(
  ...
  tomar_input:
  scanf("%c", &response);
  num = response - '1'; //num indexado en 0

  if(num>-1 && num<3){
    printf("Numeros entre 1 y %i, por favor. Volve a intentar:\n", 3);
    goto tomar_input;
  }
  ...
  switch (num) {
    ...
    break;
    case 2:
      if(ui_accusation(solution, current_player)){
        return false;
      }
    break;
  }
...
void ui_movement(
  ...
  if (pasadizo != -1 && map_occupied(map, player->location.id)){
...
void ui_suspicion(
  ...
  for(int i = 0; i < player_arr_size; i++){
    ...
    for(int j = 0; j < player_arr[it].hand.size; j++){
      if(player_arr[it].hand.data[j].id == sospecha[player_arr[it].hand.data[j].type]){
    ...
    if(tempdeck.size){
      ...
      if(opcion < 0 || opcion > tempdeck.size){
        printf("Numeros entre 0 y %d", tempdeck.size);
        goto validate_pista;
      }
